...
//        let titleOffset : CGFloat = appearance.showCircularIcon ? 0.0 : -12.0
    ...
    @discardableResult
    open func addButton(_ title:String, image: UIImage, backgroundColor:UIColor? = nil, textColor:UIColor? = nil, showTimeout:SCLButton.ShowTimeoutConfiguration? = nil, action:@escaping ()->Void)->SCLButton {
        let btn = addButton(title,image: image, backgroundColor: backgroundColor, textColor: textColor, showTimeout: showTimeout)
        btn.actionType = SCLActionType.closure
        btn.action = action
        btn.addTarget(self, action:#selector(SCLAlertView.buttonTapped(_:)), for:.touchUpInside)
        btn.addTarget(self, action:#selector(SCLAlertView.buttonTapDown(_:)), for:[.touchDown, .touchDragEnter])
    ...
    @discardableResult
    fileprivate func addButton(_ title:String,image: UIImage, backgroundColor:UIColor? = nil, textColor:UIColor? = nil, showTimeout:SCLButton.ShowTimeoutConfiguration? = nil)->SCLButton {
        // Update view height
        appearance.setkWindowHeight(appearance.kWindowHeight + appearance.kButtonHeight)
        ...
        let img = UIImageView.init(frame: CGRect.init(x: appearance.kWindowWidth*0.6, y: appearance.kButtonHeight*0.4, width: 12, height: 12))
        img.image = image
        
        btn.addSubview(img)
        btn.bringSubview(toFront: img)
